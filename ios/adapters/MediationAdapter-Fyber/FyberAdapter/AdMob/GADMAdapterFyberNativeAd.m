...
- (void)loadNativeAd {
  ADXLogEvent(ADXAdNetworkFyber, ADXAdTypeNative, ADXAdEventLoad);
  GADMAdapterFyberNativeAd *__weak weakSelf = self;
                ...
                withWatermarkData:_adConfiguration.watermark
                   withCompletion:^(IANativeAdAssets *nativeAdAssets, NSError *error) {
                     ...
                     if (!error) {
                       ...
                       ADXLogEvent(ADXAdNetworkFyber, ADXAdTypeNative, ADXAdEventLoaded);
                     } else {
                       if (error) {
                         GADMAdapterFyberLog("Failed to initialize DT Exchange SDK: %@",
                                             error.localizedDescription);
                         strongSelf->_loadCompletionHandler(nil, error);
                         ADXLogEventError(ADXAdNetworkFyber, ADXAdTypeNative, ADXAdEventLoadFailed, error.localizedDescription);
                         return;
                       }
                     }
                   }];
...
- (NSDecimalNumber *)starRating {
  return _nativeAdAssets.rating
             ? [NSDecimalNumber decimalNumberWithDecimal:_nativeAdAssets.rating.decimalValue]
             : nil;
}

// in DTExchange SDK the mediaView contains image or video, according to Ad type: Native Image Ad or
// Native Video Ad
- (UIView *)mediaView {
...
- (void)didRenderInView:(UIView *)view
  ...
  [clickableAssetViews enumerateKeysAndObjectsUsingBlock:^(GADNativeAssetIdentifier assetIdentifier,
                                                           UIView *assetView, BOOL *stop) {
...
- (void)iaNativeAdDidReceiveClick:(IANativeAdSpot *)adSpot origin:(NSString *)origin {
  ADXLogEvent(ADXAdNetworkFyber, ADXAdTypeNative, ADXAdEventClick);
  [_delegate reportClick];
}

- (void)iaNativeAdWillLogImpression:(IANativeAdSpot *)adSpot {
  ADXLogEvent(ADXAdNetworkFyber, ADXAdTypeNative, ADXAdEventShown);
...
- (void)iaNativeAdDidPresentFullscreen:(IANativeAdSpot *)adSpot {
  GADMAdapterFyberLog(@"iaNativeAdDidPresentFullscreen, unitID: %@, spotID %@",
                      adSpot.adRequest.unitID, adSpot.userInfo[@"DTSpotID"]);
}
...
- (void)iaNativeAdDidDismissFullscreen:(IANativeAdSpot *)adSpot {
  ADXLogEvent(ADXAdNetworkFyber, ADXAdTypeNative, ADXAdEventClose);
...
- (void)iaNativeAdDidExpire:(IANativeAdSpot *)adSpot {
  // as of DT Exchange SDK v8.4.0, "iaAdDidExpire" callback is triggered only from
  // IAFullscreenUnitController
  GADMAdapterFyberLog(@"iaNativeAdDidExpire, unitID: %@, spotID %@", adSpot.adRequest.unitID,
                      adSpot.userInfo[@"DTSpotID"]);
}

// native video callbacks
- (void)iaNativeAd:(IANativeAdSpot *)adSpot videoDurationUpdated:(NSTimeInterval)videoDuration {
  GADMAdapterFyberLog(
      @"iaNativeAd:videoDurationUpdated:, duration is %.2f, for unitID: %@, spotID %@",
      videoDuration, adSpot.adRequest.unitID, adSpot.userInfo[@"DTSpotID"]);
}

- (void)iaNativeAd:(IANativeAdSpot *)adSpot videoInterruptedWithError:(NSError *)error {
  GADMAdapterFyberLog(@"iaNativeAd:videoInterruptedWithError: %@, for unitID: %@, spotID %@", error,
                      adSpot.adRequest.unitID, adSpot.userInfo[@"DTSpotID"]);
}
...
- (void)iaNativeAdSpot:(IANativeAdSpot *)adSpot
    didFailToLoadImageFromUrl:(NSURL *)url
                         with:(NSError *)error {
  GADMAdapterFyberLog(
      @"iaNativeAdSpot:didFailToLoadImageFromUrl: %@, with error: %@, for unitID: %@, spotID %@",
      url.absoluteString, error, adSpot.adRequest.unitID, adSpot.userInfo[@"DTSpotID"]);
}

- (void)iaNativeAd:(IANativeAdSpot *)adSpot didLoadImageFromUrl:(NSURL *)url {
  GADMAdapterFyberLog(@"iaNativeAd:didLoadImageFromUrl %@, spotID %@", url,
                      adSpot.userInfo[@"DTSpotID"]);
}
